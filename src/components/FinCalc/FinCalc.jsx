...
const FinCalc = () => {
  ...
  const tabs = ["Retirement", "Education", "Investment", "Tax"];
    ...
    <>
      ...
      {isOpen && (
        <div className="fin-calc fixed bottom-0 left-0 right-5 lg:h-screen h-[100svh] w-full xl:p-[7rem] px-2 pt-[6rem] bg-[#EEF4EA] z-888 flex flex-col ">
          {/* Tabs */}
          <div className="flex flex-wrap  gap-4 md:mb-4 md:mt-2 xl:mb-4">
            {tabs.map((tab) => (
              <button
                key={tab}
                ...
                className={`lg:px-4 lg:py-2 px-2 py-1 rounded-full lg:text-base text-sm poppins-regular border-2 ${
                  activeTab === tab
                    ...
                    : "bg-[#2C2B2B] text-amber-50 hover:bg-[#BA8748]"
                }`}
              >
                {tab}
              </button>
          ...
          {/* Tab Content */}
          <div className="fincalc-render h-full 2xl:min-h-[75vh] bg-[#2C2B2B] p-5 rounded-xl mb-2 lg:mb-0 overflow-auto">
            {renderTabContent()}
          </div>
        </div>
      )}
